...
5. 不得直接运行 `kie_video_batch.py`，不得手动寻找或安装 `requests`，不得让用户先装 Python。启动器负责扫描、安装和修复。
...
启动器会优先使用 Codex 自带 Python，第一次调用时扫描依赖，在 `<home>/.codex/cache/h` 创建可复用隔离环境，并自动安装缺失项。之后只做快速完整性检查；插件版本或依赖变化时自动重建。不得把虚拟环境建在插件目录。
